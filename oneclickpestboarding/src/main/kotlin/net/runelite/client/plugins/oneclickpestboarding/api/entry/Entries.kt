package net.runelite.client.plugins.oneclickpestboarding.api.entry
...
class Entries {
    ...
    fun MenuOptionClicked.walkTo(point: LocalPoint) {
        try {
            val worldPoint = WorldPoint.fromLocal(client, point)
            this.menuOption = "Walk here"
            this.menuTarget = ""
            this.id = 0
            this.menuAction = MenuAction.WALK
            this.param0 = worldPoint.x
            this.param1 = worldPoint.y
            this.consume()
            val rsclient = client as RSClient
            rsclient.selectedSceneTileX = point.sceneX
            rsclient.selectedSceneTileY = point.sceneY
